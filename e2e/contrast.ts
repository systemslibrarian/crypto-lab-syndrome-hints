 ...
 *    files the node under `incomplete`. this lab builds most of its
 *    tinted surfaces with `color-mix(in oklab, ... , transparent)` — the hero's
 *    "why it matters" wash, the selected-algorithm card, and the three meter
 *    states (`--ok` 14%, `--warn` 16%, `--alarm` 16%). Text on any of those
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The 48-column H matrix,
 *    the run log and the chart data table all sit in scrolling wrappers, so at
 *    380px much of them is scrolled out of the frame. Content past the client box is not dimmed or partly drawn — it is
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This lab draws its
 *    work-factor chart as real SVG built with `createElementNS`, with axis and
 *    series labels as `<text>` siblings of the shapes they sit on — so this
 *    rule is load-bearing here. Where an `<svg>` does
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * surface in this lab is `color-mix(in oklab, ...)`. Chromium reports
     * those
     ...
     * the farthest-corner radius. This lab declares no gradients at all
     * today — the shared header's may — so the path is largely inert here and
     * kept for the case one is added. A non-gradient layer (`url()`, `none`) is unmeasurable and
     * paints nothing.
     */
     ...
     *
     * This is one very long single-page document — a 48-button hand-decode
     * row, a 48-column parity-check matrix, a growing run log and an SVG
     * chart — and every text node walks the same handful of ancestors.
     * Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has four native `<details>`, all shipping
      // CLOSED: the hand-decode toggles, the Stern-vs-Prange explainer, the
      // chart data table and the sources list. The gate scans each in both
      // configurations explicitly rather than forcing them all open at once.
     ...
     * lab builds its SVG through the typed `svg()` helper in src/ui/dom.ts,
     * which appends only Nodes, so the rule is a guard rather than a live
     * finding here.
     */
